"""
BiLSTM+Attention 金融消费者投诉分类 - Streamlit前端
端口: 8003 | emoji: 🔄
"""
...
st.set_page_config(page_title="BiLSTM+Attention 金融投诉分类", page_icon="🔄", layout="centered")
...
<div style="text-align:center;">
    <h1>🔄 金融消费者投诉文本分类系统</h1>
    <div class="subtitle">BiLSTM+Attention 循环神经网络模型</div>
</div>
""", unsafe_allow_html=True)
...
with st.expander("ℹ️ 模型简介", expanded=False):
    ...
    <p><b>模型架构</b>：Embedding → BiLSTM(双向) → Attention(注意力机制) → Dropout → FC</p>
    <p><b>模型特点</b>：双向LSTM捕捉长距离依赖关系，Attention机制自动聚焦关键信息</p>
    <p><b>参数量</b>：约 5M</p>
...
text_input = st.text_area(
    "请输入投诉文本（英文）:",
    height=150,
    placeholder="示例1：I have an issue with my checking account. The bank charged me overdraft fees...\n示例2：A collection agency is calling me about a debt I do not owe...\n示例3：I applied for a personal loan but was denied without proper explanation..."
)
    ...
    if not text_input or not text_input.strip():
        ...
        with st.spinner("预测中..."):
            try:
                ...
                st.info("启动命令：`cd 03-bilstm && python api.py`")
            except Exception as e:
                st.error(f"❌ 发生错误: {str(e)}")

st.markdown("---")
st.caption("基于 BiLSTM+Attention 的金融消费者投诉文本分类系统 | 数据集：CFPB Consumer Complaints | API端口: 8003")
