...
def test():
    ...
    if not os.path.exists(conf.model_save_path):
        raise FileNotFoundError(f"模型文件不存在: {conf.model_save_path}，请先运行 train.py")
    ...
    report_path = "../logs/test_report.txt"
    with open(report_path, "w", encoding="utf-8") as f:
        f.write(f"FinBERT 测试集评估报告\n")
        f.write(f"{'=' * 60}\n")
